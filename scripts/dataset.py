...
class GetDataset(Dataset):
    def __init__(self, data, img_size=32, is_train = True):
        self.data = data
        self.image_paths = [x[0] for x in data]
        self.labels = [x[1] for x in data]
        self.img_size = img_size
        self.hori_flip = torchvision.transforms.RandomHorizontalFlip(p=0.5)
    ...
    def __getitem__(self, index):
        ...
        # if self.is_train:
        #     image = self.hori_flip(image)
        image = torchvision.transforms.functional.resize(image, (self.img_size, self.img_size))
        image = image / 255.0
        return image, torch.Tensor([label]).float()

def get_loader(data, img_size = 32, batch_size = 32, num_workers=3, is_train = True, shuffle=True):
    dataset = GetDataset(data, img_size=img_size, is_train = is_train)
